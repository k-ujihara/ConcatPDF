...
namespace Ujihara.ConcatPDF
{
    ...
    public class EncryptOptionDialog : System.Windows.Forms.Form
    {
        ...
        {
            if (!radioNoSecurity.Checked && editMasterPassword.Text == "")
            {
        ...
        public PdfEncryptInfo Info
        {
            get
            {
                var info = new PdfEncryptInfo();
                info.Enabled = !radioNoSecurity.Checked;
                int perm = 0;
                if (radio40bitRC4.Checked) perm = EncryptionConstants.STANDARD_ENCRYPTION_40;
                if (radio128bitRC4.Checked) perm = EncryptionConstants.STANDARD_ENCRYPTION_128;
                if (checkPrinting.Checked) perm |= EncryptionConstants.ALLOW_PRINTING;
                if (checkModifyContent.Checked) perm |= EncryptionConstants.ALLOW_MODIFY_CONTENTS;
                if (checkCopy.Checked) perm |= EncryptionConstants.ALLOW_COPY;
                if (checkModifyAnnotations.Checked) perm |= EncryptionConstants.ALLOW_MODIFY_ANNOTATIONS;
                if (checkDocumentAssembly.Checked) perm |= EncryptionConstants.ALLOW_ASSEMBLY;
                if (checkFillIn.Checked) perm |= EncryptionConstants.ALLOW_FILL_IN;
                if (checkScreenReaders.Checked) perm |= EncryptionConstants.ALLOW_SCREENREADERS;
                if (checkLowPrint.Checked) perm |= EncryptionConstants.ALLOW_DEGRADED_PRINTING;
                info.Encryption = perm;
                info.UserPassword = this.editUserPassword.Text;
                info.OwnerPassword = this.editMasterPassword.Text;
            ...
            set
            {
                if (value.Enabled)
                {
                    radioNoSecurity.Checked = !value.Enabled;
                }
                else
                {
                    int v = value.Encryption & 3;
                    switch (v)
                    {
                ...
                checkPrinting.Checked = (value.Encryption & EncryptionConstants.ALLOW_PRINTING) != 0;
                checkModifyContent.Checked = (value.Encryption & EncryptionConstants.ALLOW_MODIFY_CONTENTS) != 0;
                checkCopy.Checked = (value.Encryption & EncryptionConstants.ALLOW_COPY) != 0;
                checkModifyAnnotations.Checked = (value.Encryption & EncryptionConstants.ALLOW_MODIFY_ANNOTATIONS) != 0;
                checkDocumentAssembly.Checked = (value.Encryption & EncryptionConstants.ALLOW_ASSEMBLY) != 0;
                checkFillIn.Checked = (value.Encryption & EncryptionConstants.ALLOW_FILL_IN) != 0;
                checkScreenReaders.Checked = (value.Encryption & EncryptionConstants.ALLOW_SCREENREADERS) != 0;
                checkLowPrint.Checked = (value.Encryption & EncryptionConstants.ALLOW_DEGRADED_PRINTING) != 0;

                this.editUserPassword.Text = value.UserPassword ?? "";
                this.editMasterPassword.Text = value.OwnerPassword ?? "";
            }
        }
    }
}
